...
public class Kc {
	...
	private boolean inSwitch = false; /*switch文内部にいるかの判定*/
	/**
	 * コンストラクタの生成
	 */
	...
	void parseSwitch_statement() {
		...
		for(int i = 0; i<breakAddrList.size();i++) {
			int breakAddr = breakAddrList.get(i);
			iseg.replaceCode(breakAddr, removeAddr);  /*for文のループ中にbreakを利用した場合にループからジャンプするようにする*/
		}
	...
	void parseCase_label() {
		...
		iseg.appendCode(Operator.BNE,-1); // 飛び地が確定したら
